...
- Serialize refresh in `session`. Cache the Pick. CLI 401 force-stale is under the same lock. The token HTTP call uses a background 15s timeout, not the inbound request context, and does not hold the mutex. A rotated session stays in process if `auth.json` save fails, until the file mtime is newer.
